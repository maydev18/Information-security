#include "AES.hpp"
#include <iostream>
const std::array<std::array <byte , 16> , 16> AES :: sbox = {
    {
...
const std::array<std::array <byte , 16> , 16> AES :: inv_sbox = {
    {
...
const std::array<byte , 11> AES :: rcon = {
    {0x00 , 0x01, 0x02, 0x04 , 0x08, 0x10 , 0x20 , 0x40 , 0x80, 0x1B , 0x36}
};

const block AES :: mix_col_matrix = {
    {
        {0x02, 0x03, 0x01, 0x01},
        {0x01, 0x02, 0x03, 0x01},
        {0x01, 0x01, 0x02, 0x03},
...
const block AES :: inv_mix_col_matrix = {
    {
        {0x0e, 0x0b, 0x0d, 0x09},
        {0x09, 0x0e, 0x0b, 0x0d},
        {0x0d, 0x09, 0x0e, 0x0b},
...
byte AES :: get_sbox_byte(byte b , bool inverse = 0){
...
void AES :: generate_round_keys(const std::string & s){
...
void AES :: print_block(const block & b){
...
block AES :: sub_bytes(const block & b){
    block ans;
    for(int i = 0 ; i < b.size() ; i++){
        for(int j = 0 ; j < b[0].size() ; j++){
...
block AES :: shift_rows(const block & b){
    block ans;
    for(int i = 0 ; i < b.size() ; i++){
...
block AES :: mix_columns(const block & b){
    block ans;
    for(int i = 0 ; i < b.size() ; i++){
        for(int j = 0 ; j < b[0].size() ; j++){
            byte a = 0;
...
block AES :: add_round_key(const block & b , int round){
    block ans;
    for(int i = 0 ; i < 4 ; i++){
        for(int j = 0 ; j < 4 ; j++){
...
block AES :: string_to_state(const std::string & s){
    if(s.size() != 32){
...
std::string AES :: state_to_string(const block &b){
    std::string ans = "";
    for(int j = 0 ; j < b[0].size() ; j++){
        for(int i = 0 ; i < b.size() ; i++){
...
block AES :: inv_sub_bytes(block & b){
    block ans;
    for(int i = 0 ; i < b.size() ; i++){
        for(int j = 0 ; j < b[0].size() ; j++){
...
block AES :: inv_shift_rows(block &b){
    block ans;
    for(int i = 0 ; i < 4 ; i++){
        std::array<byte , 4> tmp;
        for(int j = 0 ; j < 4 ; j++){
...
block AES :: inv_mix_columns(block& b){
    block ans;
    for(int i = 0 ; i < 4 ; i++){
        for(int j = 0 ; j < 4 ; j++){
            byte a = 0;
...
std::string AES :: decrypt(const std::string &encrypted_data){
